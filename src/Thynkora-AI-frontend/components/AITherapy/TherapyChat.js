        ...
        React.createElement("div", { className: "flex-1 overflow-y-auto px-6 py-4 space-y-4 bg-gray-100" },
            messages.map((msg, index) => (React.createElement("div", { key: index, className: `flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}` },
                React.createElement("div", { className: "relative max-w-md" },
                    React.createElement("div", { className: `px-4 py-2 rounded-lg shadow text-sm ${msg.sender === 'user'
                            ? 'bg-blue-600 text-white rounded-br-none'
                            : 'bg-gray-200 text-gray-900 rounded-bl-none'}` }, msg.text),
                    React.createElement("div", { className: `absolute w-3 h-3 ${msg.sender === 'user'
                            ? 'right-0 bottom-0 translate-x-1 rotate-45 bg-blue-600'
                            : 'left-0 bottom-0 -translate-x-1 rotate-45 bg-gray-200'}` }),
                    React.createElement("div", { className: "text-xs text-gray-400 mt-1 ml-1" }, msg.timestamp))))),
